...
# @mmaahg & ICTS_930
token = "6828686648:AAHbApTu1Kt2ji0mpMWKAar2ZSazvap73iY"
...
def start(message):
    ...
    bot.reply_to(message, f'''welcome Dear {name}In This bot 👾 spam calls via phone number 

Send phone number
With the country code followed by a + sign

Example:
+96450100756''',reply_markup=Mak().add(Btn('MORE BOTS',callback_data='click')))
...
def call(number):
    ...
    if current_time - alokt >= 60:
        ...
        if req.get('status') == 40003:
            ...
            text = f'''has been sent
Phone Number : {phonum} ✅
country code : {coucode} .'''
            return text
    else:
        remaining_time = int(60 - (current_time - alokt))
        return f'Avoid clicking on the bot, please wait {remaining_time} Seconds.'
